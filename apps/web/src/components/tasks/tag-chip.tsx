...
export function TagChip({ name, color, className, onRemove }: { name: string; color?: string | null; className?: string; onRemove?: () => void }) {
  return (
    <span className={cn("inline-flex h-5 max-w-32 items-center gap-1 rounded-full px-2 text-[11px] font-medium leading-none", `pastel-chip pastel-${asPastel(color)}`, className)}>
